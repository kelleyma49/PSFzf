...
Multiple items can be selected.  If more than one is selected by the user, the results are returned as a comma separated list (the delimiter can be customized using `Set-PsFzfOption -PSReadlineChordProviderDelimiter`).  Results are properly quoted if they contain whitespace.
